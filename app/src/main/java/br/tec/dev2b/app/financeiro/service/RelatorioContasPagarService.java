...
public class RelatorioContasPagarService {
    ...
    private static final String[] HEADERS = {"Nome", "Profissional", "Data", "Parcela", "Status", "Valor Pago", "Valor"};

    @Transactional(readOnly = true)
    public List<MovimentoFinanceiroDto> listar(UUID empresaId) {
        return repository.findByEmpresaIdAndTipoOrderByDataVencimentoDesc(empresaId, "A_PAGAR")
                .stream()
    ...
    public byte[] gerarPdf(List<MovimentoFinanceiroDto> movimentos) throws IOException {
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            doc.add(new Paragraph("Relatorio - Contas a Pagar", titleFont));
            ...
            table.setWidths(new float[]{2.5f, 2.5f, 1.8f, 1.5f, 1.5f, 1.8f, 1.8f});
            ...
            for (MovimentoFinanceiroDto m : movimentos) {
                ...
                for (int i = 0; i < row.length; i++) {
                    ...
                    cell.setHorizontalAlignment(i >= 5 ? Element.ALIGN_RIGHT : Element.ALIGN_CENTER);
                    cell.setPadding(4);
                    table.addCell(cell);
                }
            }
    ...
    public byte[] gerarExcel(List<MovimentoFinanceiroDto> movimentos) throws IOException {
        try (XSSFWorkbook wb = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            for (MovimentoFinanceiroDto m : movimentos) {
                ...
                row.createCell(4).setCellValue(m.getStatus() != null ? m.getStatus() : "-");
                Cell cPago  = row.createCell(5); cPago.setCellValue(pago.doubleValue());   cPago.setCellStyle(moneyStyle);
                Cell cValor = row.createCell(6); cValor.setCellValue(valor.doubleValue()); cValor.setCellStyle(moneyStyle);
            }

            Row totRow = sheet.createRow(rowNum);
            Cell lbl = totRow.createCell(4); lbl.setCellValue("Totais:"); lbl.setCellStyle(totalStyle);
            Cell tP = totRow.createCell(5); tP.setCellValue(totalPago.doubleValue());   tP.setCellStyle(moneyTotalStyle);
            Cell tV = totRow.createCell(6); tV.setCellValue(totalValor.doubleValue()); tV.setCellStyle(moneyTotalStyle);
    ...
    public String gerarHtml(List<MovimentoFinanceiroDto> movimentos) {
        ...
        return """
                ...
                <head>
                  <meta charset="UTF-8"/>
                  <title>Relatorio - Contas a Pagar</title>
                  <style>
                ...
                <body>
                  ...
                  <h2>Relatorio - Contas a Pagar</h2>
                  <p>Gerado em %s</p>
                  <table>
                    <thead>
                      <tr><th>Nome</th><th>Profissional</th><th>Data</th><th>Parcela</th><th>Status</th><th>Valor Pago</th><th>Valor</th></tr>
                    </thead>
                    <tbody>%s</tbody>
                    <tfoot>
                      <tr class="totals">
                ...
                """.formatted(LocalDate.now().format(DATE_FMT), rows, fmt(totalPago), fmt(totalValor));
    }
